...
for trial = 1:20
  ...
  %sc1 = [99*(1:((N_ITEMS/2)+1)), (ENCODING_TIME - N_ITEMS/2 + 1 + (1:(((N_ITEMS/2)-1))))]; %scheudle trial
  %sc1 = [(1:((N_ITEMS/2))), (N_ITEMS/2 + 99*(1:(((N_ITEMS/2)))))];  %schedule trial
  sc1 = ENCODING_TIME - (floor(ENCODING_TIME/(N_ITEMS-1)))*(0:(N_ITEMS-1)); %final oiptimal schedule
  sc1 = sort(sc1);
  
  schedule = [sc1' (1:N_ITEMS)'];
